...
public class Inventory : MonoBehaviour
{
    ...
    public void ClearInventory(CollectableData collectableData)
    {
        ...
        {
            if (inventory.Contains(item) && itemDictionary.Count > 0 && collectableData.name == "Speed"/*&& inventory.Count > 0*/)            //Removes Speed collectable both from inventory list and item dictionary
            {
                inventory.Clear();
            ...
            if (inventory.Contains(item) && itemDictionary.Count > 0 && collectableData.name == "Invisible")
            {
                inventory.Clear();
                itemDictionary.Clear();
                Debug.Log("Cleared" + " " + item.stackSize + " " + collectableData.name + "(s)");
    ...
    //Add keypress in void Update to trigger powerups in playermovement
    public bool HasSpeed(CollectableData collectableData)
    {
        if (itemDictionary.ContainsKey(collectableData))
        {
            //Debug.Log("You used " + collectableData.name);
            return true;
        }
        return false;
    }

    public bool HasInvis(CollectableData collectableData)
    {
        if (itemDictionary.ContainsKey(collectableData))
        {
            //Debug.Log("You used " + collectableData.name);
